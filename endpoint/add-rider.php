...
if (isset($_POST['register'])) {
    try {
        ...
        // Check phone number
        $stmt = $conn->prepare("SELECT COUNT(*) FROM `rider` WHERE `phone_number` = :phone_number");
        ...
        // Check email
        $stmt = $conn->prepare("SELECT COUNT(*) FROM `rider` WHERE `email` = :email");
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) FROM `rider` WHERE `driver_license_number` = :driver_license_number");
        $stmt->execute(['driver_license_number' => $driverLicenseNumber]);
        if ($stmt->fetchColumn() > 0) {
            $duplicateMsg .= 'driver License Number already exists. ';
        }

        //reg request
        // Check phone number
        $stmt = $conn->prepare("SELECT COUNT(*) FROM `rider_reg_requests` WHERE `phone_number` = :phone_number");
        $stmt->execute(['phone_number' => $phoneNumber]);
        if ($stmt->fetchColumn() > 0) {
            $duplicateMsg .= 'Phone number already exists. ';
        }
        ...
        // Check email
        $stmt = $conn->prepare("SELECT COUNT(*) FROM `rider_reg_requests` WHERE `email` = :email");
        $stmt->execute(['email' => $email]);
        if ($stmt->fetchColumn() > 0) {
            $duplicateMsg .= 'Email address already exists. ';
        }
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) FROM `rider_reg_requests` WHERE `driver_license_number` = :driver_license_number");
        $stmt->execute(['driver_license_number' => $driverLicenseNumber]);
        if ($stmt->fetchColumn() > 0) {
            $duplicateMsg .= 'driver License Number already exists. ';
        }
        ...
        if ($duplicateMsg === '') {
            ...
            <script>
                ...
                window.location.href = 'http://localhost/project/rider/rider-verification.php';
            </script>
            ";
        } else {
            echo "
            <script>
                ...
                window.location.href = 'http://localhost/project/#riderRegModal';
            </script>
            ";
        }
    } catch (PDOException $e) {
...
if (isset($_POST['verify'])) {
    try {
        ...
        if (isset($_SESSION['verification_code']) && intval($_SESSION['verification_code']) === intval($verificationCode)) {
            ...
            $insertStmt = $conn->prepare("INSERT INTO `rider_reg_requests` 
                (`first_name`, `middle_name`, `last_name`, `phone_number`, `email`, `driver_license_number`, 
                 `password`, `verification_code`) 
                VALUES (:first_name, :middle_name, :last_name, :phone_number, :email, :driver_license_number, 
                 :password, :verification_code)");
            ...
            $insertStmt->bindParam(':driver_license_number', $formData['driver_license_number'], PDO::PARAM_STR);
